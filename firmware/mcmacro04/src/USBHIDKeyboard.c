...
__xdata uint8_t MediaReport[2];  // 16bitのConsumerレポート
...
uint8_t USB_EP1_send_with_report_id(uint8_t report_id, uint8_t *data, uint8_t len) {
  if (UsbConfig == 0) return 0;

  while (UpPoint1_Busy);
  Ep1Buffer[64] = report_id;  // Report ID
  for (uint8_t i = 0; i < len; i++) {
    Ep1Buffer[65 + i] = data[i];
  }
...
// Consumer Page Usage ID (HID Usage Tablesより)
#define HID_CONSUMER_VOLUME_UP     0x00E9
#define HID_CONSUMER_VOLUME_DOWN   0x00EA
#define HID_CONSUMER_MUTE          0x00E2

__xdata uint8_t MediaReport[2];       // 16bit Consumer Control Report
__xdata uint16_t lastMediaKeycode = 0; // 現在押下中のキー記録

uint8_t MediaKey_press(uint16_t keycode) {
  lastMediaKeycode = keycode;
  MediaReport[0] = keycode & 0xFF;
  MediaReport[1] = keycode >> 8;
  return USB_EP1_send_with_report_id(0x02, MediaReport, 2);
}

uint8_t MediaKey_release(void) {
  if (lastMediaKeycode == 0) return 0;  // 既にリリース済み
